...
export async function cancelAppointment(formData) {
  ...
  try {
    ...
    await db.$transaction(async (tx) => {
      ...
      // Add transaction log for patient
      await tx.creditTransaction.create({
        data: {
          userId: appointment.patientId,
          amount: 2,
          type: "ADMIN_ADJUSTMENT", // ✅ must match your enum
        },
      });

      // Add transaction log for doctor (optional, depending on logic)
      await tx.creditTransaction.create({
        data: {
...
export async function markAppointmentCompleted(formData) {
   ...
   include:{
    patient:true,
       ...
       console.log("Actual appointment status:", appointment.status);
