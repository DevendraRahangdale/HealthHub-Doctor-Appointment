...
if(user.transactions.length >0){
    ...
    //if we already allocate credits for this month and the plan is the same,just return 
...
export async function deductCreditsForAppointment(userId,doctorId) {
    try {
        ...
        if(user.credits< APPOINTMENT_CREDIT_COST ){
            throw new Error("Insufficient credits to book an appoinment");
        }
...
// update patients credits balance (decrement)
const updatedUser = await tx.user.update({
    where:{
        id:user.id,
    },
...
// update doctor's credits balance (increment)
